...
d <- decisions |>
  ...
  select(land, citizen, decisions, pop, time) |>
  filter(
    citizen %in% c(
      "Ukraine",
      "Palestine*",
...
p <- d |>
  ...
  labs(
    title = "Þróun heildarfjölda verndarveitinga frá 2008 til {closest_state}",
    subtitle = str_c(
  ...
  transition_states(
    ar,
    wrap = FALSE, transition_length = 1, state_length = 0.01
  ) +
...
anim_save(
  animation = p_anim,
  filename = "Figures/decisions_anim.mp4"
)
...
anim_save(
  animation = p_anim,
  filename = "Figures/decisions_anim.gif"
)
